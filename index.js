...
app.post("/processAndDeployVideo", async (req, res) => {
  ...
  try {
    ...
    // STEP 2: PURE COMPRESSION (Preserving native aspect ratio & metadata)
    console.log("Compressing video while preserving native metadata...");
    execSync(`ffmpeg -i "${rawFilePath}" -c:v libx264 -pix_fmt yuv420p -profile:v main -crf 30 -preset fast -r 30 -c:a aac -b:a 64k -ac 1 -movflags +faststart "${finalFilePath}" -y`);
